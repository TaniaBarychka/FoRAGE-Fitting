...
ar3<-lm(L.Fitted.a..median.of.BS.samples.~L.PredBM+L.PreyBM+temp_A, data=df_na)
summary(df_na)
...
ar3_opt<-lm(L.Fitted.a..median.of.BS.samples.~L.PredBM+temp_A, data=df_na)
summary(ar3_opt)
...
ggPredict(ar3_opt, interactive = TRUE)
...
**Ln(Attack Rate)=0.85X+(-7.01+0.90 per Arrhenius term), where X=Ln Predator Body mass**

So, the activcation energy of attack rate for the full model is *Ea* = 0.90.
...
m2<-update(m1,~ . - L.PredBM:L.PreyBM:Habitat)
summary(m2)

m3<-update(m2,~. -L.PreyBM:Habitat)
summary(m3)

m4<-update(m3,~. -L.PredBM:temp_A)
summary(m4)

m5<-update(m4,~.-L.PreyBM:temp_A)
summary(m5)

m6<-update(m5,~.-L.PreyBM)
summary(m6)
...
m3_terr<-update(m2_terr, ~.-df_Terr$L.PreyBM)
summary(m3_terr)
...
The model's fit is truly terrible.

Let's try a simpler model.

```{r echo=FALSE,fig.width=10, fig.height=10}

m4_terr<-lm(df_Terr$L.Fitted.a..median.of.BS.samples.~df_Terr$L.PredBM+df_Terr$L.PreyBM+df_Terr$temp_A)
...
#try to remove predator body mass
m5_terr<-update(m4_terr,~. -df_Terr$L.PreyBM)
summary(m5_terr)
plot(m5_terr)

```

Removing prey BM increases BM from 1368  to 1373. Optimal for terrestrial, Prey bodymass is significant at 0.01. The optimal is **L.Fitted.a..median.of.BS.samples. ~ L.PredBM +L.PreyBM +temp_A**. However, the model is rearly poor with a lot of trends and skew.

Without prey body mass the model is : **Th=0.75x+(-6.65+0.74xTempA)**.
...
m5_terr_na<-lm(L.Fitted.a..median.of.BS.samples.~L.PredBM+temp_A, data=df_Terr_na)
...
m3_aq<-update(m2_aq, ~.-df_Aq$L.PreyBM:df_Aq$temp_A)
summary(m3_aq)
...
m5_aq<-update(m4_aq, ~. -df_Aq$L.PredBM:df_Aq$L.PreyBM)
summary(m5_aq)
...
Aquatic models are much better behaved than terrestrial. THe model with Prey:Pred interaction is better than without ar 0.01.  Both models aslo have a three-way interaction.

Let's try a simpler model.

```{r echo=FALSE,fig.width=10, fig.height=10}

m6_aq<-lm(df_Aq$L.Fitted.a..median.of.BS.samples.~df_Aq$L.PredBM+df_Aq$L.PreyBM+df_Aq$temp_A)

summary(m6_aq)
drop1(m6_aq)

m7_aq<-update(m6_aq,~.-df_Aq$L.PreyBM)
summary(m7_aq)
...
A relatively well-behaved model (aquatic): **Th=0.88x+(-7.06+1.08xTemp_A)**.
